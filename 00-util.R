...
get_sim_df <- function(method = 'stochastic', plist = parlist){  ## can also be 'stochastic'
  readRDS(sprintf('rds/R0-%.2f_%s_%s.rds', 
                  plist$R0_vals[1], 
                  plist$model_type, 
                  method))$sim_df %>%
...
get_shape <- function(mean, vv){
  rate = mean/vv
  shape = mean*rate
  stopifnot(equal_ish(shape/rate, mean))
  stopifnot(equal_ish(shape/rate^2, vv))
  shape
}
