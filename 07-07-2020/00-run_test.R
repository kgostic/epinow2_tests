...
run_test <- function(parlist,  # List of parameters used to generate synthetic data
  ...
  if(length(d_case_dist) == 0){d_case_dist <- function(x){dlnorm(x, obs_rep_delay$mean, obs_rep_delay$sd)}}
  if(length(d_death_dist) == 0){d_death_dist <- function(x){dlnorm(x, death_rep_delay$mean, death_rep_delay$sd)}} # Additional delay from symptoms -> observation
  ...
  if(length(r_case_dist) == 0){r_case_dist <- function(n){rlnorm(n, obs_rep_delay$mean, obs_rep_delay$sd)}}
  if(length(r_death_dist) == 0){r_death_dist <- function(n){rlnorm(n, death_rep_delay$mean, death_rep_delay$sd)}} # Additional delay from symptoms -> observation
