...
cat(
  sprintf('The report shows results from %s tests', params$path)
)
```
...
## Get parameters for input in to EpiNow2 inference

```{r}
testpars <- readRDS(sprintf('%s/%s/testpars.rds', params$dir_date, params$path))
input_table <- data.frame(
  true = c(unlist(testpars[grepl('true', names(testpars))]), parlist$true_mean_GI, sqrt(parlist$true_var_GI)),
  input = unlist(testpars[grepl('input', names(testpars))]),
  parameter = rep(c('mean', 'sd'), 4),
...
```{r results = "asis", strip.white= FALSE}
cat(sprintf('![](%s/%s/synthetic_obs.png)', params$dir_date, params$path))
```
## Run inference
...
# cat('***\n
#     ### Deaths - Rt estimates\n')
# cat(sprintf('\n![Rt estimates vs. truth](figs/rt-%s-deaths.png)\n', params$path))
# 
# ## Show plots of inferred infection time series
# cat('\n### Deaths - inferred infections\n')
# cat(sprintf('\n![Inferred infection time series vs. true infections and observations](figs/infections-%s-deaths.png)\n', params$path))
# 
# ## Show plot of posteriors vs. priors vs. truth
# cat('\n### Deaths - posterior vs. prior\n')
# cat(sprintf('\n![Prior vs. posterior vs. truth for all key distributions](figs/priors_vs_post-%s-death.png)\n', params$path))
# 
# ## Show pairs plot
# cat('\n### Deaths - pairs plots of last 300 posterior samples\n')
# cat(sprintf('\n![Pairs plot of key distribution params](figs/pairs-%s-deaths.png)\n', params$path))
```
