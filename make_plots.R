...
plot_pr_vs_post <- function(
  ...
  gg <- ggplot()+
    geom_line(data = gi_prior, aes(x = xx, y = true))+
    geom_ribbon(data = gi_post, aes(x = xx, ymin = `lower.2.5%`, ymax = `upper.97.5%`), fill = 'yellow', alpha = .5)+
    geom_line(data = gi_post, aes(x = xx, y = mean), color = 'goldenrod', alpha = .5)+
    geom_line(data = gi_prior, aes(x = xx, y = prior), lty = 2, color = 'orange') +
  ...
  ggsave2(filename = sprintf('%s/priors_vs_post-%s.png', path, delay_type), width = 8, height = 4, units = 'in', dpi = 300)
}
...
merge(
  ...
  ggsave2(filename = sprintf('%s/rt-%s.png', path, delay_type), width = 5, height = 4, units = 'in', dpi = 300)
}
