...
testpars <- list(
  last_obs_time = 150,
  output_folder = 'misspec_flat_delay',
  ## True delays
...
lines(xx, dunif(xx, 0, testpars$true_mean_case_delay*2), col = 'red', lty = 2)
...
lines(xx, dunif(xx, 5, testpars$true_mean_death_delay*2-5), col = 'red', lty = 2)
legend('topright', c('input', 'true'), col = c(1, 'red'), lty = 1)
...
outpatient_delay_dist <- function(nn, inc_dist = r_inc_dist){
  r_sym_to_obs_dist <- function(n){runif(n, 0, testpars$true_mean_case_delay*2)}
  inc_dist(nn) + r_sym_to_obs_dist(nn)
}

death_delay_dist <- function(nn, inc_dist = r_inc_dist){
  r_sym_to_obs_dist <- function(n){runif(n, 5, testpars$true_mean_death_delay*2-5)} # Additional delay from symptoms -> observation
  inc_dist(nn) + r_sym_to_obs_dist(nn)
}
